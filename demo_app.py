...
class XPopupDemo(BoxLayout):
    def __init__(self, **kwargs):
        super(XPopupDemo, self).__init__(padding=20, spacing=15,
                                         orientation='vertical', **kwargs)

        pnl1 = BoxLayout(spacing=15)
        pnl1.add_widget(ButtonEx(id='msgbox', text='XMessage demo',
                                 on_release=self._on_click))
        pnl1.add_widget(ButtonEx(id='confirm', text='XConfirmation demo',
                                 on_release=self._on_click))
        pnl1.add_widget(ButtonEx(id='error', text='XError demo',
                                 on_release=self._on_click))
        pnl1.add_widget(ButtonEx(id='progress', text='XProgress demo',
                                 on_release=self._on_click))
        self.add_widget(pnl1)

        pnl1 = BoxLayout(spacing=15)
        pnl1.add_widget(ButtonEx(id='input', text='XTextInput demo',
                                 on_release=self._on_click))
        pnl1.add_widget(ButtonEx(id='notes', text='XNotes demo',
                                 on_release=self._on_click))
        pnl1.add_widget(ButtonEx(id='slider', text='XSlider demo',
                                 on_release=self._on_click))
        pnl1.add_widget(ButtonEx(id='login', text='XAuthorization demo',
                                 on_release=self._on_click))
        self.add_widget(pnl1)

        pnl1 = BoxLayout(spacing=15)
        pnl1.add_widget(ButtonEx(text='XOpenFile demo',
                                 on_release=self._open_dialog_demo))
        pnl1.add_widget(ButtonEx(text='XSaveFile demo',
                                 on_release=self._save_dialog_demo))
        pnl1.add_widget(ButtonEx(text='XFolder demo',
                                 on_release=self._folder_dialog_demo))
        self.add_widget(pnl1)
